...
{
    /**
     * Selection de la table Prospects
     */
    ...
    {
        $requete = $this->connection->prepare("SELECT id, nom, prenom, adresse, code_postal, ville, commentaire FROM prospects WHERE id=:id");
    ...
    {
        $nom = isset($_POST['parm0']) ? $_POST['parm0'] : null;
        $prenom = isset($_POST['parm1']) ? $_POST['parm1'] : null;
        $adresse = isset($_POST['parm2']) ? $_POST['parm2'] : null;
        ...
        $requete = $this->connection->prepare("INSERT INTO prospects VALUES (NULL, :nom, :prenom, :adresse, :code_postal, :ville, :commentaire)");
    ...
    public function updateDB()
    {
        ...
        $requete = $this->connection->prepare(
            "UPDATE `prospects` SET nom=:nom, prenom=:prenom, adresse=:adresse, code_postal=:code_postal, ville=:ville, commentaire=:commentaire WHERE id=:id"
        );
        $requete->bindParam(':nom', $nom);
        $requete->bindParam(':prenom', $prenom);
        $requete->bindParam(':adresse', $adresse);
    ...
    {
        $id = isset($_POST['parm0']) ? $_POST['parm0'] : null;
        $requete = $this->connection->prepare("DELETE FROM prospects WHERE id=:id");
        $requete->bindParam(':id', $id);
        $result = $requete->execute();
        return $result;
    }
